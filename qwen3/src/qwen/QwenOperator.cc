...
bgemxGQA::bgemxGQA(float _alpha, int num_q_head, int num_kv_head) { 
    this->alpha = _alpha; 
    groupsize = num_q_head / num_kv_head;
}

void bgemxGQA::forward(Matrix3D<float> &A, MatrixView<float> &B, Matrix3D<float> &output) {
...
void bgemxGQA::forward_ink_kernel_qk(Matrix3D<float> &A, MatrixView<float> &B, Matrix3D<float> &output)
{
    int bs = A.m_dim_x;
    int m = A.m_dim_y;
    int n = B.m_dim_y;
...
void bgemxGQA::forward_ink_kernel_pv(Matrix3D<float> &A, MatrixView<float> &B, Matrix3D<float> &output) {
...
void bgemxGQA::forward_weight_untransposed(Matrix3D<float> &A, MatrixView<float> &B,
