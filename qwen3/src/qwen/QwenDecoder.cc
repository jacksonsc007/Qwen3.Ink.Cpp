...
Qwen3Model_Output Qwen3Model::forward(const struct Qwen3Model_Input &input) {
    ...
    */
    ...
    // ---
    PROFILE_START(forward_profile_name + "::embedding");
    Matrix3D<float> token_embedding  = wte.forward(input.input_ids);
    PROFILE_END(forward_profile_name + "::embedding");
    IF_DEBUG_IO(
    ...
    // ---
    PROFILE_START(forward_profile_name + "::attention mask generation");
    ...
    PROFILE_END(forward_profile_name + "::attention mask generation");

    // ---
    // 3rd stage: layer-by-layer inference
    // ---
    PROFILE_START(forward_profile_name + "::decoder layers");
    ...
    PROFILE_END(forward_profile_name + "::decoder layers");

    // ---
    // 4th stage: output layernorm
    // ---
    PROFILE_START(forward_profile_name + "::output layernorm");
    Matrix3D<float> last_hidden_states = output_norm.forward(hidden_states);
    PROFILE_END(forward_profile_name + "::output layernorm");
    IF_DEBUG_DECODER(
