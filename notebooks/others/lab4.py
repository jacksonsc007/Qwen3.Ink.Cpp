...
def pseudo_quantize_model_salient_weight_fp16(
    ...
    for n, m in model.named_modules():
        if isinstance(m, nn.Linear):
            ...
            topk_num = int(0.01 * importance.size(0))
            outlier_indices = torch.topk(importance, k=topk_num, dim=0)[1]
...
def auto_scale_block(module, name, w_bit,
    ...
    def _search_module_scale(block, linears2scale: list, x, kwargs={}):
        ...
        # find mean of each channel of input
        s_x = x.view(-1, x.shape[-1]).abs().mean(0) # (d, )
        ...
        for ratio in range(n_grid):
            ...
            scales = (s_x ** ratio+1e-6)
            assert scales.shape == s_x.shape

            ############### YOUR CODE ENDS HERE #################

            # print(f"before norm: ratio: {ratio} , scales: min={scales.min()} max={scales.max()}")
            scales = scales / (scales.max() * scales.min()).sqrt().view(1, -1)

            # print(f"after norm: ratio: {ratio} , scales: min={scales.min()} max={scales.max()}")
            for fc in linears2scale:
                ...
                # INK: in-place operation?
                assert torch.isnan(fc.weight.data).sum() == 0
                ...
                assert torch.isnan(fc.weight.data).sum() == 0
                assert torch.isinf(fc.weight.data).sum() == 0
    ...
    print(f"[INFO] final_scales for attention input: {final_scales}")
    ...
    print(f"[INFO] final_scales for attention output: {final_scales}")
    ...
    print(f"[INFO] final_scales for fc1: {final_scales}")
    ...
    print(f"[INFO] final_scales for fc2: {final_scales}")
