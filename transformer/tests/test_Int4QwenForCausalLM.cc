...
void test_model_correctness()
{
    ...
    std::string int4_param_path = "../../INT4/models/qwen-7b-chat";
    std::string param_path = "../../qwen-7b-chat"; // the path that store the gt from python model.
    Int4QwenForCausalLM model = Int4QwenForCausalLM(int4_param_path, qwen_config);
    ...
    Int4QwenForCausalLM_input input = {input_ids};
    ...
    Int4QwenForCausalLM_output output = model.forward(input);
    ...
    Int4QwenForCausalLM_input input_2nd = {input_ids_2nd, output.past_keys, output.past_values};

    success = 0;
    printf("\e[31m[INFO]\e[m start testing QwenForCausalLM with KV Cache...\n");
    Int4QwenForCausalLM_output output_2nd = model.forward(input_2nd);
