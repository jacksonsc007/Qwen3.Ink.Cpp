...
void test_model_correctness()
{
    ...
    Int4QwenBlock_input input(hidden_states, attention_mask);
    // evaluate
    ss.str("");
    ss << int4_param_path << "layer" << layer_idx << "/";
    std::string model_path = ss.str();
    Int4QwenBlock layer(model_path, config, layer_idx);
    Int4QwenBlock_output output = layer.forward(input);
