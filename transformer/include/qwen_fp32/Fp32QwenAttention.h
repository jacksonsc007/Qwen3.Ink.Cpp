...
struct Fp32Qwen3Attention_Input
{
    ...
    Fp32Qwen3Attention_Input(Matrix3D<float> hidden_states_, Matrix3D<float> attention_mask_, int layer_idx_)
        : hidden_state(hidden_states_), attention_mask(attention_mask_), layer_idx(layer_idx_) {}

    Fp32Qwen3Attention_Input(
...
class Fp32Qwen3Attention {
public:
    // construct
    Fp32Qwen3Attention(){}
    Fp32Qwen3Attention(std::string param_path, const struct qwen3_config config);

    // member
    int hidden_dim, head_dim, max_sqlen;
    int num_q_head, num_kv_head;
    int q_dim, kv_dim;
    LinearFp32 k_proj, v_proj, q_proj, o_proj;
    Qwen3RMSNorm q_norm, k_norm;
    RotaryPosEmb rope_embed;
    BMM_F32T qk_bmm, pv_bmm;
    std::string profile_name = "Fp32Qwen3Attention";

    // method
    static void initialize_memory(const struct qwen3_config config);
    struct Fp32Qwen3Attention_Output forward(const struct Fp32Qwen3Attention_Input &input);
};