...
void QwenSiLuMul(Matrix3D<float> a, Matrix3D<float> b) {
...
Fp32QwenBlock::Fp32QwenBlock(std::string param_path, const struct qwen_config config, int layer_idx)
{
    ...
    if (layer_idx == 0)
    {
        ...
        Fp32QwenAttention::initialize_memory(config);
    }
    ...
    this->ln_1 = QwenRMSNorm(ln_1_weight_mat);
    float *ln_2_weight;
    ...
    this->ln_2 = QwenRMSNorm(ln_2_weight_mat);
    this->embed_dim = embed_dim_;
    this->hidden_dim = hidden_dim_;
    this->num_attention_heads = num_heads_;
    this->layer_idx = layer_idx;
    this->attn = Fp32QwenAttention(param_path + "/self_attn", config);
    float *gate_proj_weight, *down_proj_weight, *up_proj_weight;
    allocate_aligned_memory(gate_proj_weight, embed_dim_ * hidden_dim_ * sizeof(float));
    allocate_aligned_memory(up_proj_weight, hidden_dim_ * embed_dim_ * sizeof(float));
    allocate_aligned_memory(down_proj_weight, embed_dim_ * hidden_dim_ * sizeof(float));
    this->gate_proj = Linear_FP(
        Matrix3D<float>(gate_proj_weight, 1, hidden_dim_, embed_dim_), (param_path + "/w2/weight.bin")
    );
    this->up_proj   = Linear_FP(
        Matrix3D<float>(up_proj_weight, 1, hidden_dim_, embed_dim_), (param_path + "/w1/weight.bin")
    );
    this->down_proj = Linear_FP(
        Matrix3D<float>(down_proj_weight, 1, embed_dim_, hidden_dim_), (param_path + "/c_proj/weight.bin")
    );
}


Fp32QwenBlock_output Fp32QwenBlock::forward(const Fp32QwenBlock_input &input)
{
    ...
    // -----------------------------
    // 6th stage: wrapping up
    // -----------------------------
    struct Fp32QwenBlock_output output(residual_out, attn_output.attn_probs_reshaped,
