    ...
    const struct qwen_params generation_config, std::string tiktoken_path, bool interactive, qwen_config config)
{
    ...
    // ===========================
        Int4QwenForCausalLM *model = static_cast<Int4QwenForCausalLM *>(model_ptr);
        Int4QwenForCausalLM_input model_input;
        Int4QwenForCausalLM_output model_output;
        ...
        memcpy(logits.data(), &model_output.logits.m_data[(sqlen - 1)*vocab_size], vocab_size*sizeof(float));
        has_past_kv = true;
    // ===========================
