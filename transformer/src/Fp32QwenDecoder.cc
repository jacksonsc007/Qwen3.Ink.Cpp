...
Matrix3D<float> Fp32QwenModel::prepare_decoder_attention_mask(int length, int past_length)
{
...
Fp32QwenModel::Fp32QwenModel(std::string param_path, const struct qwen_config config){
    ...
    this->ln_F = QwenRMSNorm(ln_F_weight);
    // decoder layers
    ...
    for (int layer_idx = 0; layer_idx < config.num_layers; layer_idx++) {
        ...
        Fp32QwenBlock layer = Fp32QwenBlock(path, config, layer_idx);
        this->layers.push_back(layer);
    }
}

Fp32QwenModel_output Fp32QwenModel::forward(const struct Fp32QwenModel_input &input) {
    ...
    for (int i = 0; i < this->layers.size(); i++)
    {
        ...
        if (!input.has_past_keys_values)
        {
            struct Fp32QwenBlock_input layer_input = {hidden_states, causal_attn_mask};
            struct Fp32QwenBlock_output layer_output = this->layers[i].forward(layer_input);
        ...
        {
            struct Fp32QwenBlock_input layer_input = {hidden_states, causal_attn_mask, input.past_keys[i], input.past_values[i]};
            struct Fp32QwenBlock_output layer_output = this->layers[i].forward(layer_input);
            hidden_states = layer_output.hidden_states;
            past_keys.push_back(layer_output.past_key_value.first);
            past_values.push_back(layer_output.past_key_value.second);
    ...
    // -----------------------------
    struct Fp32QwenModel_output output  = {last_hidden_states, past_keys, past_values}; 
