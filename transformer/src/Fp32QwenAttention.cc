...
void Fp32QwenAttention::reshape_headfirst(Matrix3D<float> unshape, Matrix3D<float> shaped, int sqlen)
{
...
void Fp32QwenAttention::reshape_seqfirst(Matrix3D<float> shape1, Matrix3D<float> shape2, int sqlen)
{
...
void Fp32QwenAttention::initialize_memory(const struct qwen_config config) {
...
Fp32QwenAttention::Fp32QwenAttention(std::string param_path, const struct qwen_config config)
{
    ...
    allocate_aligned_memory(q_bias, 1 * config.embed_dim * sizeof(float));
    allocate_aligned_memory(k_bias, 1 * config.embed_dim * sizeof(float));
    allocate_aligned_memory(v_bias, 1 * config.embed_dim * sizeof(float));
    allocate_aligned_memory(out_weight, config.embed_dim * config.embed_dim * sizeof(float));
    // TODO: check the correspondence btw weight name and offline weight file.
    this->q_proj =
        Qwen_Linear_FP(
            Matrix3D<float>(q_weight, 1, config.embed_dim, config.embed_dim), param_path + "/q_proj/weight.bin",
            Matrix3D<float>(q_bias, 1, config.embed_dim, 1), param_path + "/q_proj/bias.bin"
        );
    this->k_proj =
        Qwen_Linear_FP(
            Matrix3D<float>(k_weight, 1, config.embed_dim, config.embed_dim), param_path + "/k_proj/weight.bin",
            Matrix3D<float>(k_bias, 1, config.embed_dim, 1), param_path + "/k_proj/bias.bin"
        );
    this->v_proj =
        Qwen_Linear_FP(
            Matrix3D<float>(v_weight, 1, config.embed_dim, config.embed_dim), param_path + "/v_proj/weight.bin",
            Matrix3D<float>(v_bias, 1, config.embed_dim, 1), param_path + "/v_proj/bias.bin"
        );
    this->out_proj =
        Linear_FP(Matrix3D<float>(out_weight, 1, config.embed_dim, config.embed_dim), param_path + "/o_proj/weight.bin");
...
struct Fp32QwenAttention_output Fp32QwenAttention::forward(const struct Fp32QwenAttention_input &input)
{
    PROFILE_START(profile_name);
    struct Fp32QwenAttention_output output;
